...
function FaqComponent({ imageSrc, altText , messageTitle, messageBody, text, customStyles = "" }) {
  ...
  return (
    <div className={`flex   rounded-[12px]  px-[40px] gap-[10px] ${customStyles}`}>
      ...
      {messageTitle && <p className="font-bold">{messageTitle}</p>}
        <p>{messageBody}</p>
        {text &&<p className="border border-[#755AEE] text-[#755AEE] px-[8px] py-[4px] rounded-full">{text}</p>}
        </div>
